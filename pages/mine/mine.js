...
Page({
  ...
  onLoad:function(params){
    ...
    if(publisherId != null && publisherId != '' && publisherId != undefined){
        ...
        if(user.id != publisherId){
          wx.request({
            url: app.serverUrl+'/user/isfollowing?userId='+publisherId+'&fanId='+user.id,
            method:'POST',
            header:{
  ...
  followMe:function(res){
    var me = this;
    var type = res.currentTarget.dataset.followtype;
    var user = app.getGlobalUserInfo();
    var userId = user.id;
    var url = ''
    if(type == '1'){
      url = '/user/following?userId='+ me.publisher + '&fanId='+userId;
    }else{
      url = '/user/unfollow?userId=' + me.publisher + '&fanId=' + userId; 
    }
